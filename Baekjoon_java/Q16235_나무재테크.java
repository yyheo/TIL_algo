...
public class Q16235_나무재테크 {
	...
	public static void main(String[] args) throws IOException {
		...
		while(K-- != 0 && trees.length != 0) {
			...
			// ArrayList 인덱스 0에 추가, remove, 돌때마다 sort 해주는거 => 다 시간초과
			// 결국 배열 만들어서 배열 맨앞에 넣어주는걸로...
			Tree[] treestmp = new Tree[tmp.size() + cnt];
			for (int i = 0; i < tmp.size(); i++) { 
				treestmp[i] = tmp.get(i);
			}
			int tmpcnt = tmp.size();
			for (int i = 0; i < trees.length; i++) {
				Tree cur = trees[i];
